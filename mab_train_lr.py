...
'''def main() -> None:
    ...
    # 2. batch=16: 如果 GPU 内存允许，增大 batch 可以提升训练速度（默认 16）
    # 3. workers=4: 如果内存允许，可以增加到 4（单进程时，并行时保持 2）
    base_args = {
        "device": device,
        "workers": 4,         # 单进程时可以增加到 4，并行时建议保持 2
        ...
        "batch": 16,          # 如果 GPU 内存允许，可以尝试 16 或更大（默认 16）
    }
...
def main() -> None:
    ...
    if args.device is None:
        ...
        if torch.cuda.is_available():
            device = "0"  # 使用第一个 GPU
            print(f"[MAB] 检测到 NVIDIA GPU: {torch.cuda.get_device_name(0)}，将使用 GPU 训练")
        elif hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
            device = "mps"
            print(f"[MAB] 检测到 Apple Silicon GPU，将使用 MPS 训练")
        else:
            device = "cpu"
            print("[MAB] 未检测到 GPU，将使用 CPU 训练（速度较慢）")
    else:
        ...
        if device == "0" or device.startswith("cuda"):
            if torch.cuda.is_available():
                ...
                print(f"[MAB] 警告：用户指定了 {device}，但未检测到 NVIDIA GPU")
                # 检查是否有Apple GPU
                if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                    print(f"[MAB] 检测到 Apple GPU，将使用 MPS")
                    device = "mps"
                else:
                    print(f"[MAB] 将回退到 CPU")
                    device = "cpu"
        elif device == "mps":
            if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                print(f"[MAB] 使用用户指定的设备: {device} (Apple GPU)")
            else:
                print(f"[MAB] 警告：用户指定了 MPS，但未检测到 Apple GPU，将回退到 CPU")
                device = "cpu"
        else:
            print(f"[MAB] 使用用户指定的设备: {device}")
    ...
    if device == "mps":
        # Apple MPS 优化配置
        workers = 0  # MPS必须workers=0
        batch_size = 8  # Apple内存较小，减少batch size
        print(f"[MAB] MPS设备优化: workers={workers}, batch={batch_size}")
    elif device == "0" or device.startswith("cuda"):
        # NVIDIA GPU配置
        workers = 8
        batch_size = 16
        print(f"[MAB] CUDA设备优化: workers={workers}, batch={batch_size}")
    else:
        # CPU配置
        workers = 4  # CPU使用较少workers
        batch_size = 4  # CPU内存有限
        print(f"[MAB] CPU设备优化: workers={workers}, batch={batch_size}")

    # 这里可以从现有 train.py 复用一些通用超参，例如 workers、imgsz 等
    base_args = {
        "device": device,
        "workers": workers,        # 根据设备类型动态设置
        "optimizer": "SGD",        # 关闭 optimizer=auto，确保 lr0/lrf 生效
        "cache": False,            # 不使用快取（内存和磁盘空间有限时的最佳选择）
        "plots": False,            # 关闭 YOLO 自带训练图，避免额外开销
        "imgsz": 512,              # 将输入尺寸从 640 降到 512，减少 GFLOPs、提升速度
        "batch": batch_size,       # 根据设备类型设置batch size
    }
    
    # 如果是MPS设备，添加额外优化参数
    if device == "mps":
        base_args.update({
            "half": False,          # MPS可能不支持half精度
            "close_mosaic": 10,     # 提前关闭mosaic以节省内存
            "warmup_epochs": 3.0,   # 适当的热身epochs
        })
        print(f"[MAB] 添加MPS额外优化参数: half={base_args.get('half', 'auto')}, "
              f"close_mosaic={base_args.get('close_mosaic', 'auto')}")
